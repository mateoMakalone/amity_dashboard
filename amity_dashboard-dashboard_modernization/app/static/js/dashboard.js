...
// Флаг, указывающий на первую загрузку дашборда. Он используется,
// чтобы отображать индикатор загрузки и рендерить секции только один раз.
let firstLoad = true;
...
async function loadSectionsData() {
    // Показываем индикатор загрузки и рендерим секции только при первой загрузке
    let isFirst = false;
    try {
        const container = document.getElementById('sections-container');
        isFirst = firstLoad && container && !container.hasChildNodes();
        ...
        // Скрываем индикатор загрузки и сбрасываем флаг firstLoad только после первой загрузки
        if (isFirst) {
            hideLoading();
            firstLoad = false;
        }
